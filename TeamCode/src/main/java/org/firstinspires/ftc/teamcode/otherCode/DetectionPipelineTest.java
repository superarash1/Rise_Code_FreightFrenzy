...
public class DetectionPipelineTest extends OpenCvPipeline {
 Telemetry telemetry;
   ...
   public int yellowContourCount = 0;
    ...
    List<MatOfPoint> BiggestContour;
    ...
    public String getContourPosition() {
        return contourPositionString;
    }
    ...
    */
        ...
        contourPositionString = "";
        double maxVal = 0;
        int maxValIdx = 0;
        for (int i = 0; i < yellowContours.size(); i++)
        {
            double contourArea = Imgproc.contourArea(yellowContours.get(i));
        ...
        Imgproc.drawContours(all, BiggestContour, maxValIdx, GOLD); //input
        for (int i = 0; i < BiggestContour.size(); i++) {

            // Filters out contours with an area less than 50 (defined in the filter contours method)

                yellowRect.add(Imgproc.boundingRect(BiggestContour.get(i)));
                Imgproc.rectangle(all, yellowRect.get(yellowContourCount), GOLD, 2);
        ...
        for (int i = 0; i < yellowRect.size(); i++) {
            Rect rect = yellowRect.get(i);
            contourPositionString += String.format("Yellow Contour %d: (%7d, %7d) ", i, rect.x + rect.width / 2, rect.y + rect.height / 2);
        }
